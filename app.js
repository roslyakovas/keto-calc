...
window.resetProductDatabase = () => Products.resetToDefaults(); // ✅ Исправлено имя функции
...
window.addRecipeIngredientRow = () => {
  ...
  if (container) {
    ...
    // ✅ Исправлен template string и innerHTML
...
// ✅ ДОБАВЛЕНА отсутствующая функция (исправляет ошибку "UI.selectProductForMeal is not a function")
window.UI = window.UI || {};
window.UI.selectProductForMeal = function(productId, mealId = null) {
  console.log('🛒 Выбор продукта для приёма пищи:', { productId, mealId });
  
  const product = AppState.data.products.find(p => p.id == productId || p.name === productId);
  if (!product) {
    UI.showToast('⚠️ Продукт не найден');
    return;
  }
  
  // Если mealId не указан, берём текущий активный приём пищи
  const targetMealId = mealId || AppState.data.activeMealId || AppState.data.meals[0]?.id;
  if (!targetMealId) {
    UI.showToast('⚠️ Нет доступных приёмов пищи');
    return;
  }
  ...
  Meals.addProduct(targetMealId, {
    ...
    name: product.name,
    amount: 100, // вес по умолчанию 100г
  ...
  UI.showToast(`✅ ${product.name} добавлен в план`);
  Dashboard.update();
  UI.renderAll();
};
    UI.closeModal('recipeModal');
